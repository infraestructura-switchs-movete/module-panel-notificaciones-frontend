...
import { getOrders, deleteOrder, updateOrderStatus } from "../Api/Pedido";
...
function PanelPedidos() {
  ...
  useEffect(() => {
    cargarPedidos(true);
    const interval = setInterval(() => cargarPedidos(false), 100);
              ...
              {pedido.products.map((producto) => (
                ...
                >
                  <span className="font-medium">
                    {producto.quantity} x {producto.productName}
                  </span>
                  <span className="font-bold">
                    {formatearPrecio(producto.price * producto.quantity)}
                  </span>
                </div>
              ))}
            </div>
